# fpi-agent UI Preflight

This preflight is the shared UI safety net for fpi-agent. It runs the real Next.js app in a visible Chromium browser by default and mocks backend/proxy responses at the network layer, so the checks do not depend on local API keys, a running FastAPI server, or a remote tunnel.
...
```bash
FPI_AGENT_UI_HEADLESS=true npm run preflight:ui
```
...
The complete workflow suite is the stricter user-journey layer on top of the broader smoke preflight. These tests preserve browser state across navigation and assert both visible GUI outcomes and the backend payloads the UI submits.
...
The office and error-state suite covers heavier artifact/file paths and negative flows that should stay recoverable in the GUI.
...
The conversation scale suite covers long-running conversation behavior that must match real user motion through the GUI.
...
The natural office workflow suite covers the day-to-day employee prompts that fpi-agent claims to handle. These are GUI workflows with normal workplace language, uploaded Office/PDF files, persisted assistant output, artifact cards, and same-thread follow-up. The suite rejects test-marker prompts such as `WF_*` or "assistant answer must..." so the flow stays close to real user motion.
...
The edge-state regression suite covers recoverability and remote edge states.
...
The deep surface suite covers the claimed desktop feature surfaces that are easy to miss in happy-path smoke tests. It is desktop-only because these controls live in the desktop shell; mobile has dedicated remote workflows above.
