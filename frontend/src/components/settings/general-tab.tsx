...
export function GeneralTab() {
  ...
  useEffect(() => {
    ...
    setAccountName(user?.display_name?.trim() || user?.email || "员工");
  ...
  const checkForUpdate = useCallback(async () => {
    ...
    setUpdateError(null);
    try {
      const update = await enterpriseUpdate.checkNow();
      if (!update) {
        throw new Error("检查更新失败");
      }
      if (update.update_available) {
        setUpdateVersion(update.latest_version);
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="flex items-center justify-between gap-3 rounded-xl border border-[var(--border-default)] bg-[var(--surface-secondary)] px-4 py-3">
          ...
          <Button
            variant="outline"
            size="sm"
            className="h-8 shrink-0 text-ui-caption text-[var(--color-destructive)]"
