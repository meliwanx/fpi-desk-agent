"""fpi-agent agent adapter — bridges the message bus to fpi-agent's generation pipeline.

Replaces nanobot's AgentLoop: consumes InboundMessage from the bus,
runs fpi-agent's full agent (run_generation), and publishes OutboundMessage.
"""
...
class AgentAdapter:
    """Consumes messages from the bus and runs fpi-agent's agent pipeline.
    ...
    async def _handle(self, msg: InboundMessage) -> None:
        """Process a single inbound message through fpi-agent's agent."""
    ...
    async def _call_agent(self, msg: InboundMessage) -> str:
        """Call fpi-agent's run_generation pipeline and collect the response text."""
